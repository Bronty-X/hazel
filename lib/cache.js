...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    if (token && typeof token === 'string' && token.length > 0) {
      headers.Authorization = `\sBearer ${token}`
    }
    ...
    const { status, body } = await retry(
      async () => {
        ...
        */
        if (response.status !== 200) {
          throw new Error(
            `Tried to cache RELEASES, but failed fetching ${url} , status ${response.status} , headers ${JSON.stringify(headers)} , body ${await response.text()}`
          )
        }
        return response
      },
